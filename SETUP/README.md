...
1.  Create the GitHub repository for this deployment to live in.
    ...
    -   Set the repo name to: **seattle-staging**
...
1.  Create (or re-use) a
    [Google Cloud billing account](https://console.cloud.google.com/billing/linkedaccount?project=cdp-seattle-staging-dbengvtn)
    and attach it to the newly created project (cdp-seattle-staging-dbengvtn).
...
1.  Generate a Google Service Account JSON Key for your Google Cloud Project.
    ...
    add a file called `cdp-seattle-staging-dbengvtn.json` to it
    (i.e. `.keys/cdp-seattle-staging-dbengvtn)`. This file will be used later on.
...
1.  Attach the Pulumi Access Token and the
    ...
    1. Pulumi Access Token -- Create a [new secret](https://github.com/CouncilDataProject/seattle-staging/settings/secrets/actions/new)

    -   Set the name to: **PULUMI_ACCESS_TOKEN**
    -   Set the value to: The token you kept from step #2
    -   Click "Add secret"

    2. Google Service Account JSON -- Create a [new secret](https://github.com/CouncilDataProject/seattle-staging/settings/secrets/actions/new)

    -   Set the name to: **GOOGLE_CREDENTIALS**
    -   Set the value to: the contents of the file `.keys/cdp-seattle-staging-dbengvtn.json`
    -   Click "Add secret"
...
1.  Initialize and push the local repository to GitHub.
    ...
    ```bash
    git remote add origin https://github.com/CouncilDataProject/seattle-staging.git
    ```
...
1.  Once the
    ["Web App" GitHub Action Successfully Complete](https://github.com/CouncilDataProject/seattle-staging/actions?query=workflow%3A%22Web+App%22)
    configure GitHub Pages.

    Go to your repository's [GitHub Pages Configuration](https://github.com/CouncilDataProject/seattle-staging/settings/pages)
...
1.  Once the
    ["Infrastructure" GitHub Action Successfully Completes](https://github.com/CouncilDataProject/seattle-staging/actions?query=workflow%3A%22Infrastructure%22)
...
1.  Once the
    ...
    [Direct Link to Enable](https://console.cloud.google.com/apis/api/speech.googleapis.com/data_logging?project=cdp-seattle-staging-dbengvtn)
...
1.  Once the
    ...
    -   Navigate to [Firebase Console](https://console.firebase.google.com),
        login to the Google Account you used during step #2, select the `cdp-seattle-staging-dbengvtn` Firebase project
...
**If all steps complete successful your web application will be viewable at: https://CouncilDataProject.github.io/seattle-staging**
...
Navigate and follow the instructions in the the file: `python/cdp_seattle_staging_backend/scraper.py`.
